...
let rec internal formatASTRec (node: AST.Node<'E,'T>): Tree =
    ...
    | ArrayElementAssign(array, index, value) ->
        mkTree $"ArrayElementAssign" node [
            ("array", formatASTRec array)
            ("index", formatASTRec index)
            ("value", formatASTRec value)
        ]
    | While(cond, body) ->
    ...
    //This code is used to format the AST node for the ForScoped structure, making it more readable.
    //mkTree "For scoped" node [...] creates a tree structure representation.
    //For each part—init, cond, update, and body—it calls the formatASTRec function to recursively format them.
    //The result is a structured, readable representation of the ForScoped loop.
        mkTree "For scoped" 
            node 
            [("init", formatASTRec init);
                ("cond", formatASTRec cond);
    ...
    | ForEach(loopVar, array, body) ->
        mkTree $"For each %s{loopVar}" node [
                                               ("array", formatASTRec array)
                                               ("body", formatASTRec body)]                   
    | Array(length, data) -> 
    ...
    //its main purpose is to visualize the AST, turning the abstract tree structure into a more understandable text format. 
    //If the Parser creates the correct AST but the Pretty Printer is wrong, 
    //even though the AST is correct, it won’t be displayed properly.
            mkTree $"ArraySlice" node [("arr", formatASTRec arr)
                                       ("start", formatASTRec start)
                                       ("end", formatASTRec ending)]
    | MutableStructExpr(fields) ->
    /// Formatted fields of the mutable structure
        let fieldsChildren =
            List.map (fun (isMutable, f, n) -> 
                let mutabilityPrefix = if isMutable then "mutable " else ""
                ($"%s{mutabilityPrefix}field %s{f}", formatASTRec n)
                ) fields
        mkTree "MutableStruct" node fieldsChildren                                                                     
    | Lambda(args, body) ->

...
let rec internal doCodegen (env: CodegenEnv) (node: TypedAST): Asm =
    ...
    | Var(name) ->
        ...
        | _ ->  // Default case for variables holding integer-like values
            ...
            | Some(Storage.Label(lab)) ->
                Asm(RV.LA(Reg.r(env.Target), lab), $"Load variable '%s{name}'")
            // in stack, load address from stack frame pointer
            | Some(Storage.Frame(offset)) ->
    ...
    | DoWhile(cond, body) ->
        (doCodegen env body) 
        ++ (doCodegen env {node with Expr = While(cond, body)})
    ...
    | ForScoped(letMutVar, cond, update, body) ->
        // Generate code for the initialization of the mutable variable
        let initCode = doCodegen env letMutVar
        
        // Extend the environment to include the mutable variable's storage
        let mutableVarStorage =
            match letMutVar.Expr with
            | LetMut(name, _, _) ->
                env.VarStorage.Add(name, Storage.Reg(Reg.r(env.Target)))
            | _ -> env.VarStorage
        
        // New environment with the mutable variable included
        let extendedEnv = { env with VarStorage = mutableVarStorage }
        
        // Generate code for the while loop using the mutable variable
        let loopBody = {node with Expr = While(cond, {node with Expr = Seq([body; update])})}
        let loopCode = doCodegen extendedEnv loopBody
        
        // Combine the initialization code and loop code
        initCode ++ loopCode
    ...
    | Array(length, data) ->
        ...
        let mark_label = Util.genSymbol $"mark_label"
        let arraybeginLabel = Util.genSymbol "array_loop_begin"
        //check array length
        let Array = (doCodegen env length) 
                        ++ Asm([
                                RV.BNEZ(Reg.r(env.Target), mark_label), "Jump if length not 0"                                
                                RV.LI(Reg.a7, 93), "RARS syscall: Exit2"
                                RV.LI(Reg.a0, assertExitCode), "Load exit code"
                                RV.ECALL, ""
                                
                                RV.LABEL(mark_label), "length ok"
                            ])
                            //Memory Allocation for length and address
                        ++ (beforeSysCall [Reg.a0] [])
                            .AddText([
                                (RV.LI(Reg.a0, 2 * 4), "Memory allocate")
                                (RV.LI(Reg.a7, 9), "RARS syscall: Sbrk")
                                (RV.ECALL, "")
                                (RV.MV(Reg.r(env.Target), Reg.a0), "Move syscall result to target")
                            ])
                        ++ (afterSysCall [Reg.a0] [])
                            //restore metadata to t6, length
                        ++ Asm(RV.MV(Reg.t6, Reg.r(env.Target)), "Move to t6") ++ (doCodegen env length).AddText([
                                RV.SW(Reg.r(env.Target), Imm12(4), Reg.t6), " "
                                RV.MV(Reg.t4, Reg.r(env.Target)), "Move to t4"
                            ])
                            //restore element
                        ++ (beforeSysCall [Reg.a0] [])
                            .AddText([
                                (RV.LI(Reg.a0, 4), " ") 
                                (RV.MUL(Reg.a0, Reg.a0, Reg.r(env.Target)), "* 4")
                                (RV.LI(Reg.a7, 9), "RARS syscall: Sbrk")
                                (RV.ECALL, "")
                                (RV.MV(Reg.r(env.Target + 2u), Reg.a0), "Move syscall result to target")
                            ])
                        ++ (afterSysCall [Reg.a0] [])
                            //restore address to t6
                        ++ Asm(RV.SW(Reg.r(env.Target + 2u), Imm12(0), Reg.t6), " ").AddText([
                                RV.MV(Reg.t5, Reg.r(env.Target + 2u)), "start t5"
                            ])
                        ++ (doCodegen env data)
                            .AddText([
                                RV.LI(Reg.a2, 4), "Load the element size set 4"
                                RV.LI(Reg.a3, 0), "Load the index"
                            ])
                            .AddText(RV.LABEL(arraybeginLabel))
                            .AddText([
                            RV.MUL(Reg.r(env.Target + 2u), Reg.a2, Reg.a3), "Offset calculation"                          
                            RV.ADD(Reg.r(env.Target + 3u), Reg.t5, Reg.r(env.Target + 2u)), "Element address calculation"
                            RV.SW(Reg.r(env.Target), Imm12(0), Reg.r(env.Target + 3u)), "storage the value"
                            RV.ADDI(Reg.a3, Reg.a3, Imm12(1)), "Index increment"
                            RV.BLT(Reg.a3, Reg.t4, arraybeginLabel), "Loop continue check"
                            RV.MV(Reg.r(env.Target), Reg.t6), "Struct address update"
                        ]).AddText(RV.COMMENT("Array finish"))
        Array
  
    | ArrayElement(target, index) ->

        let indexCode = (doCodegen env index)
        let lengthlabel1 = Util.genSymbol $"array_index_ok"
        let lengthlabel2 = Util.genSymbol $"array_index_ok"
        //load address and length
        let ArrayElement = (doCodegen env target)
                            .AddText([
                                        RV.LW(Reg.r(env.Target + 1u), Imm12(0), Reg.r(env.Target)), "address"
                                        RV.LW(Reg.r(env.Target + 4u), Imm12(4), Reg.r(env.Target)), "length"
                                    ])
                            // check index boundary 
                            ++ indexCode
                            ++ Asm([
                                        RV.BLT(Reg.r(env.Target), Reg.r(env.Target + 4u), lengthlabel1), "Check index < length"
                                        RV.LI(Reg.a7, 93), "RARS syscall: Exit2"
                                        RV.LI(Reg.a0, assertExitCode), "Load exit code"
                                        RV.ECALL, ""

                                        RV.LABEL(lengthlabel1), "Index ok"
                                    ])
                            ++ Asm([
                                        RV.LI(Reg.a7, 0), ""
                                        RV.BGE(Reg.r(env.Target), Reg.a7, lengthlabel2), "Check index >= 0"
                                        // RV.BNEZ(Reg.r(env.Target), lengthlabel2), "Jump if not 0"       
                                        RV.LI(Reg.a7, 93), "RARS syscall: Exit2"
                                        RV.LI(Reg.a0, assertExitCode), "Load exit code"
                                        RV.ECALL, ""

                                        RV.LABEL(lengthlabel2), "Index ok"
                                    ])
                            //caculate address and load value
                            ++ Asm([
                                        (RV.LI(Reg.t3, 4), "")
                                        (RV.MUL(Reg.r(env.Target), Reg.r(env.Target), Reg.t3), "Offset calculation")
                                        (RV.ADD(Reg.r(env.Target), Reg.r(env.Target), Reg.r(env.Target + 1u)), "Element address calculation")
                                        (RV.LW(Reg.r(env.Target), Imm12(0), Reg.r(env.Target)), "Load array element")
                                    ]).AddText(RV.COMMENT("ArrayElement finish"))
        ArrayElement 
    
    | ArrayLength(target) ->
        (doCodegen env target).AddText([
                (RV.LW(Reg.r(env.Target), Imm12(4), Reg.r(env.Target)), "Array length")
            ])
    ...
    | ArraySlice(target, start, ending) -> 
        ...
        let check_start = Util.genSymbol $"start_index_ok"
        let check_end = Util.genSymbol $"end_index_ok"

        let arrayslice = (doCodegen env start)
                            //check start index boundary
                            ++  Asm([
                                        RV.LI(Reg.a7, 0), ""
                                        RV.BGE(Reg.r(env.Target), Reg.a7, start_index_ok), "Check start index >= 0"
                                        RV.LI(Reg.a7, 93), "RARS syscall: Exit2"
                                        RV.LI(Reg.a0, assertExitCode), ""
                                        RV.ECALL, ""
                                        RV.LABEL(start_index_ok), "start index ok"
                                    ])
                            ++ endIndex.AddText([
                                        RV.SUB(Reg.r(env.Target + 3u), Reg.r(env.Target + 1u), Reg.r(env.Target)), "caculate difference"
                                    ])
                            //check length
                            ++ Asm([
                                        RV.BNEZ(Reg.r(env.Target + 3u), length_ok), "Jump if not 0"     
                                        RV.LI(Reg.a7, 93), "RARS syscall: Exit2"
                                        RV.LI(Reg.a0, assertExitCode), ""
                                        RV.ECALL, ""
                                        RV.LABEL(length_ok), "length ok"
                                    ])
                            // allocate memory
                            ++ (beforeSysCall [Reg.a0] [])
                                .AddText([
                                    ...
                                    (RV.LI(Reg.a7, 9), "RARS syscall: Sbrk")
                                    (RV.ECALL, "")
                                    (RV.MV(Reg.r(env.Target + 1u), Reg.a0), "Move syscall result to target + 1u")
                                ])
                            ++ (afterSysCall [Reg.a0] [])
                            ++ Asm(RV.MV(Reg.t6, Reg.r(env.Target + 1u)), "Move to t6")
                                .AddText([
                                    RV.SW(Reg.r(env.Target + 3u), Imm12(4), Reg.t6), "store length"
                                    RV.MV(Reg.t4, Reg.r(env.Target)), "Move to t4"
                                ])
                            //check index2 boundary
                            ++ (doCodegen env start).AddText([
                                    RV.LW(Reg.r(env.Target + 2u), Imm12(4), arrayReg), ""
                                    RV.BLE(Reg.r(env.Target), Reg.r(env.Target + 2u), check_start), "start index < array length"
                                    RV.LI(Reg.a7, 93), "RARS syscall: Exit2"
                                    RV.LI(Reg.a0, assertExitCode), ""
                                    RV.ECALL, ""
                                    RV.LABEL(check_start), "start index ok"
                                ])
                            ++ (doCodegen env ending).AddText([
                                RV.LW(Reg.r(env.Target + 2u), Imm12(4), arrayReg), ""
                                RV.BLE(Reg.r(env.Target), Reg.r(env.Target + 2u), check_end), "end index < array length"
                                RV.LI(Reg.a7, 93), "RARS syscall: Exit2"
                                RV.LI(Reg.a0, assertExitCode), ""
                                RV.ECALL, ""
                                RV.LABEL(check_end), "end index ok"
                            ])
                            ++ Asm([
                                    RV.LI(Reg.a0, 4), "element size, 4 bytes" 
                                    RV.MUL(Reg.t4, Reg.t4, Reg.a0), "calculate the offset" 
                                    RV.ADD(Reg.t5, Reg.t5, Reg.t4), "Add offset to get the address" 
                                    RV.SW(Reg.t5, Imm12(0), Reg.t6), "Store the address to t6"
                                    RV.MV(Reg.r(env.Target), Reg.t6), "Move to the target register"
                                ])
        arrayslice                    
    ...
    | Application(expr, args) ->
        ...
        let extraRegisters =
            if args.Length > 8
            then List.map (fun i -> Reg.r(env.Target + (uint i) + 1u)) [8..(args.Length-1)]
            else [] // stack, map the indice to corresponding registers

        let argsSaveCode = saveRegisters (List.rev extraRegisters) []
        ...
        callCode
            ...
            .AddText(RV.COMMENT("Restore extra arguments"))
                  ++ (restoreRegisters (List.rev extraRegisters) [])
            ++ retCode

...
let rec subst (node: Node<'E,'T>) (var: string) (sub: Node<'E,'T>): Node<'E,'T> =
    //subst function:
    //This function performs variable substitution. 
    //It traverses the AST, replacing all free occurrences of a specified variable with a given expression.
    ...
    | ForScoped(init, cond, update, body) ->
      //In subst, it substitutes variables in the initialization, condition, update, and body. 
      //In freeVars and capturedVars, it computes variables from all four parts.
        {node with Expr = ForScoped(
                            (subst init var sub), //In the initialization expression init, 
                            //replace all free occurrences of the variable var with the expression sub
                            (subst cond var sub),
                            (subst update var sub),
                            (subst body var sub))}
                            
    | ForEach(loopVar, array, body) when loopVar = var ->
        //This branch handles the case where the variable being substituted (var) is the same as the loop variable.
        //ForEach: In subst, it substitutes variables in the array and body, but not the loop variable. 
        //In freeVars and capturedVars, it computes variables from the array and removes the loop variable from the body.
        {node with Expr = ForEach(loopVar,
                              (subst array var sub),
                              body)}
    | ForEach(loopVar, array, body) -> //This is the general case where var is not the same as loopVar.
        {node with Expr = ForEach(loopVar,
                              (subst array var sub),
                              (subst body var sub))}
    ...
    | MutableStructExpr(fields) ->
        let (mutabilities, fieldNames, initNodes) = List.unzip3 fields
        let substInitNodes = List.map (fun e -> (subst e var sub)) initNodes
        {node with Expr = MutableStructExpr(List.zip3 mutabilities fieldNames substInitNodes)}          
    ...
    | ArraySlice(arr, start, ending) -> 
        //In all three functions, it handles the target array, start index, and end index.
...
let rec freeVars (node: Node<'E,'T>): Set<string> =
    //freeVars function:
    //This function computes the set of free variables in an AST node. Free variables are those not bound in the current scope.
    match node.Expr with
    | UnitVal
    | IntVal(_)
    | BoolVal(_)
    ...
    | ForScoped(init, cond, update, body) ->
        Set.unionMany [freeVars init; freeVars cond; freeVars update; freeVars body] 
    | ForEach(loopVar, array, body) ->
        Set.union (freeVars array)
                  (Set.remove loopVar (freeVars body))                                                           
    | Struct(fields) ->
...
let rec capturedVars (node: Node<'E,'T>): Set<string> =
    //capturedVars function:
    //This function computes the set of captured variables in an AST node. 
    //Captured variables are those used in an inner scope but defined in an outer scope.
    match node.Expr with
    | UnitVal
    | IntVal(_)
    | BoolVal(_)
    ...
    | ForScoped(init, cond, update, body) ->
        Set.unionMany [capturedVars init; capturedVars cond; capturedVars update; capturedVars body]
    | ForEach(loopVar, array, body) ->
        Set.union (capturedVars array)
                  (Set.remove loopVar (capturedVars body))  

...
let rec subst (node: Node<'E,'T>) (var: string) (sub: Node<'E,'T>): Node<'E,'T> =
    ...
    | ForScoped(loopVar, init, cond, update, body) when loopVar = var ->
        // Do not substitute the variable in the loop body
        {node with Expr = ForScoped(loopVar, 
                                    (subst init var sub),
                                    (subst cond var sub),
                                    (subst update var sub),
                                    body)}
    | ForScoped(loopVar, init, cond, update, body) ->
        // Propagate the substitution in all parts, including the loop body
        {node with Expr = ForScoped(loopVar,
                                    (subst init var sub),
                                    (subst cond var sub),
                                    (subst update var sub),
...
let rec capturedVars (node: Node<'E,'T>): Set<string> =
    ...
    | ForScoped(loopVar, init, cond, update, body) ->
        Set.union (capturedVars init)
                  (Set.union (capturedVars cond)
                             (Set.union (capturedVars update) 
